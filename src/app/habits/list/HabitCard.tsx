    ...
    <Link href={`/habits/${habit.id}`}>
      <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider ${diff.bg} ${diff.color}`}
          >
            {diff.label}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1 mt-auto">
          {selectedDays.map((day, idx) => (
            <div
              key={idx}
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-black transition-colors ${
                day.active ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-50 text-slate-300'
              }`}
            >
              {day.label}
            </div>
